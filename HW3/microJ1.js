...
function identList() {
    ...
    while (tok == COMMA)  {   //eşittir geldiyse değerle atama işlemi için with value çalışır
       match(COMMA);
	   list.push(withValue()); 
    }
    return list;
...
function withValue() {
    let i = identifier();
	if(tok.kind!=ASSIGN) //gelen eşittir değil ise identifierı döndür
	{
		return i;
	}
    match(ASSIGN); //eşitse match et
    let f = factor();
    return new WithValue(i,f); //withvalue classında değerler set et
}
